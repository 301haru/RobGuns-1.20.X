...
public class DragonsBreathProjectile extends GunProjectile implements ItemSupplier {

    public DragonsBreathProjectile(EntityType<? extends Projectile> pEntityType, Level pLevel) {
    ...
    public void makeParticle(int pParticleAmount) {
        if (pParticleAmount > 0) {
            for(int j = 0; j < pParticleAmount; ++j) {
                this.level().addParticle(ParticleTypes.FLAME, this.getRandomX(0.5D), this.getRandomY(), this.getRandomZ(0.5D),
                        0, 0, 0);
            }
        }
    }
